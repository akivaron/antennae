...
除上述属性以外，Login.Nis 还支持 antd.Input 的所有属性，并且自带默认的基础配置，包括 `placeholder` `size` `prefix` 等，这些基础配置均可被覆盖。
## Login.Password、Login.Mobile 同 Login.Nis
...
除上述属性以外，Login.Captcha 支持的属性与 Login.Nis 相同。
