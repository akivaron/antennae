(function (global, factory) {
    ...
    Object.keys(filterJSON).forEach(key => {
      ...
      if (filterValue != null && filterValue.length) {
        sampleList = sampleList.filter(item => {
          ...
          return String(item[key]) === String(filterValue) || String(item[key]).indexOf(String(filterValue)) > 0;
        });
      }
    });
    return res.json({
  ...
  var user = {
    ...
    'POST /api/login/account': (req, res) => {
      ...
      if (password === 'codetrial' && userName === 'admin') {
        res.send({
          status: 'ok',
          type,
      ...
      if (password === 'codetrial' && userName === 'user') {
        res.send({
          status: 'ok',
          type,
  ...
  const data = Object.assign({},geographic,notices,sample,user);
